...
impl PreparedWheelSource<'_> {
    ...
    pub fn install(
        ...
        let installation_seconds = installation_started.elapsed().as_secs_f64();
        ...
        match fs::symlink_metadata(&path) {
            Ok(metadata) => {
                if metadata.file_type().is_symlink()
                    || !metadata.is_file()
                    || fs::read(&path)? != include_bytes!("../../handoff/wheel_source.py")
                {
                    return Err("staged WheelSource bridge changed after creation".into());
                }
            }
            Err(error) if error.kind() == std::io::ErrorKind::NotFound => {
                write_new(&path, include_bytes!("../../handoff/wheel_source.py"))?;
            }
            Err(error) => return Err(error.into()),
        }
        Ok(path)
